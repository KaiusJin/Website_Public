...
import {regions,regionText,GROUND_Y,MOBILE_HOTSPOT_Y} from './data/regions';
...
export default function JourneyApp(){
 ...
 bridge.current.reduced=reduced;bridge.current.onInteract=openRegion;bridge.current.touch=touch;
 ...
 },[lang,reduced,touch,cleanScreen]);
 ...
 return <main className={`journey-root ${world.region>4?'journey-night':''} ${touch?'has-touch':''}`} onPointerDownCapture={()=>setBlurred(false)} onClickCapture={releasePointerFocus} onFocusCapture={()=>setBlurred(false)} data-paused={String(bridge.current.pause)} data-player-x={world.x} data-player-y={world.y} data-player-mode={world.mode} data-player-facing={world.facing??'side'} data-velocity-x={world.vx} data-velocity-y={world.vy} data-fps={world.fps} data-room={world.room||'outdoors'} data-region={region.id}>
  ...
  {!cleanScreen&&ready&&started&&!panel&&<aside className="chapter-caption" key={region.id}><span>{t.chapter} {String(world.region+1).padStart(2,'0')} <i> / 07</i></span><h1>{chapterTitle}</h1><p>{scene?.description||regionText(region,'description',lang)}</p></aside>}
  ...
  {!cleanScreen&&ready&&started&&!panel&&!world.room&&!world.transitioning&&world.atRight&&world.next!==null&&<button className="route-sign route-sign-right" onClick={()=>travel(world.next)} aria-label={`${t.visit}: ${regionText(regions[world.next],'title',lang)}`}><span>{regionText(regions[world.next],'title',lang)}</span><Icon name="arrow"/></button>}
  {!cleanScreen&&ready&&started&&!panel&&!world.room&&!world.transitioning&&world.atLeft&&world.previous!==null&&world.previous!==undefined&&<button className="route-sign route-sign-left" onClick={()=>travel(world.previous)} aria-label={`${t.visit}: ${regionText(regions[world.previous],'title',lang)}`}><Icon name="arrow"/><span>{regionText(regions[world.previous],'title',lang)}</span></button>}
