...
export default function JourneyApp(){
 const host=useRef(null),bridge=useRef({stick:{x:0,y:0},pause:true}),game=useRef(null);
 ...
 const [reduced,setReduced]=useState(()=>savedBool('journey-reduced',matchMedia('(prefers-reduced-motion:reduce)').matches));
 ...
 const openRegion=useCallback(id=>{const r=regions.find(r=>r.id===id);if(r)setPanel(r.section);},[]);
 ...
 useEffect(()=>{
  const resize=()=>{setPortrait(matchMedia('(orientation:portrait)').matches&&(navigator.maxTouchPoints>0||matchMedia('(pointer:coarse)').matches));clear();};
 ...
 useEffect(()=>{
  ...
  try{localStorage.setItem('journey-language',lang);localStorage.setItem('journey-reduced',JSON.stringify(reduced));localStorage.setItem('journey-touch',JSON.stringify(touch));}catch{/* Optional preferences. */}
 },[lang,reduced,touch]);
 const travel=index=>{clear();bridge.current.jumpTo?.(index);setPanel(null);setStarted(true);};
 const begin=()=>{setStarted(true);host.current?.focus();};
 ...
 return <main className={`journey-root ${world.region>4?'journey-night':''} ${touch?'has-touch':''}`} onPointerDownCapture={()=>setBlurred(false)} onFocusCapture={()=>setBlurred(false)} data-paused={String(bridge.current.pause)} data-player-x={world.x} data-player-y={world.y} data-player-mode={world.mode} data-region={region.id}>
  <div ref={host} className="journey-canvas" tabIndex={-1} aria-label={t.brand}/>
  <div className="journey-vignette" aria-hidden="true"/>
  <header className="journey-header">
   <a className="journey-brand" href="/" title={t.classic}><span className="brand-seal"><Icon name="moon" size={23}/></span><span><strong>{t.brand}</strong><small>{t.subtitle}</small></span></a>
   <nav aria-label={t.map}><button onClick={()=>setPanel('personal')}><Icon name="book"/><span>{t.journal}</span></button><button onClick={()=>setPanel('map')}><Icon name="map"/><span>{t.map}</span></button><span className="nav-divider"/><button className="language-button" onClick={()=>setLang(lang==='en'?'zh-CN':'en')} aria-label={t.language}>{lang==='en'?'中':'EN'}</button><button className="settings-button" onClick={()=>setPanel('settings')} aria-label={t.settings}><Icon name="settings"/></button><a href="/" className="classic-link">{t.classic}<Icon name="external" size={14}/></a></nav>
  </header>
  {ready&&!started&&<section className="journey-intro"><p className="intro-eyebrow"><span/> {t.eyebrow}</p><h1>{t.hero.split('\n').map((line,i)=><span key={i}>{line}</span>)}</h1><p className="intro-body">{t.intro}</p><button className="journey-primary" onClick={begin}>{t.begin}<Icon name="arrow"/></button></section>}
  ...
  {ready&&started&&!panel&&!portrait&&world.hotspots.map(h=>{const r=regions.find(r=>r.id===h.id);return <button className={`world-hotspot ${world.near===h.id?'is-near':''}`} key={h.id} style={{left:h.x,top:h.y}} onClick={()=>openRegion(h.id)} aria-label={`${t.interact}: ${t[r.section]}`}><span className="hotspot-symbol"><Icon name={r.icon} size={23}/></span><span>{t[r.section]}</span><small>{world.near===h.id?'E / ↵':'✧'}</small></button>;})}
  {ready&&!panel&&<footer className="journey-footer"><div className="journey-position"><span className="position-dot"/>{world.mode==='walking'?t.walking:world.mode==='landing'?t.landing:t.flying}<small>{touch?t.touchHelp:world.mode==='walking'?t.help:t.flyHelp}</small></div><div className="mini-map" aria-label={t.map}>{regions.map(r=><button key={r.id} className={r.index===world.region?'active':r.index<world.region?'visited':''} onClick={()=>travel(r.index)} aria-label={`${t.visit}: ${regionText(r,'title',lang)}`} aria-current={r.index===world.region?'step':undefined}><span/><small>{String(r.index+1).padStart(2,'0')}</small></button>)}</div><button className="next-chapter" onClick={()=>travel((world.region+1)%7)} aria-label={t.next}><Icon name="arrow"/></button></footer>}
  {ready&&touch&&started&&!panel&&!portrait&&<TouchControls bridge={bridge} t={t} mode={world.mode}/>}
  ...
  {panel&&!portrait&&<Modal title={title} eyebrow={panel==='map'?`01 — 07 · ${t.brand}`:t.brand} onClose={close} t={t} lang={lang} onLanguage={()=>setLang(lang==='en'?'zh-CN':'en')} wide={panel==='map'}>
   {panel==='map'?<div className="travel-map">{regions.map(r=><button className={`map-destination ${r.index===world.region?'current':''}`} key={r.id} onClick={()=>travel(r.index)}><div className="map-thumb" style={{backgroundImage:`url(${r.image})`}}><span>{String(r.index+1).padStart(2,'0')}</span><Icon name={r.icon} size={28}/></div><div><small>{t[r.section]}</small><h3>{regionText(r,'title',lang)}</h3><p>{regionText(r,'description',lang)}</p></div><Icon name="arrow"/></button>)}</div>:panel==='settings'?<><div className="setting-row"><label htmlFor="motion"><strong>{t.reduced}</strong><span>{t.reducedHelp}</span></label><input id="motion" type="checkbox" checked={reduced} onChange={e=>setReduced(e.target.checked)}/></div><div className="setting-row"><label htmlFor="touch"><strong>{t.touch}</strong><span>{t.touchSetting}</span></label><input id="touch" type="checkbox" checked={touch} onChange={e=>setTouch(e.target.checked)}/></div><h3>{t.controls}</h3><dl className="controls-list"><div><dt>A / D · ← / →</dt><dd>{t.step}</dd></div><div><dt>SPACE</dt><dd>{t.jump}</dd></div><div><dt>F</dt><dd>{t.fly} / {t.land}</dd></div><div><dt>W / S · ↑ / ↓</dt><dd>{t.ascend}</dd></div><div><dt>E · ENTER</dt><dd>{t.interact}</dd></div></dl><p>{t.pause}</p><p className="fine-print">{t.saved}<br/>{t.credits}</p><a className="journal-text-button" href="/">{t.classic}<Icon name="external"/></a></>:<ContentPanel section={panel} content={content} lang={lang} t={t} onPersonal={()=>setPanel('personal')} onRestart={()=>travel(0)}/>}
  </Modal>}
 </main>;
}
