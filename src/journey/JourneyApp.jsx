...
import {regions,sceneText,MOBILE_HOTSPOT_Y,libraryProjectHotspots,libraryOtherHotspot} from './data/regions';
...
export default function JourneyApp(){
 ...
 const profile=content.data.site_profile[0];
 const welcomeIntro=profile?.[lang==='en'?'journey_en':'journey_zh']?.welcome_intro;
 const openRegion=useCallback(id=>{
  if(id==='library-door'){bridge.current.enterLibrary?.();return;}
  if(id==='library-exit'){bridge.current.exitLibrary?.();return;}
  if(id===libraryOtherHotspot.id){setPanel('other-projects');return;}
  const projectPoint=libraryProjectHotspots.find(point=>point.id===id);
  if(projectPoint){const project=bridge.current.projects?.[projectPoint.index];if(project){setProjectId(project.id);setPanel('project-detail');}return;}
  if(experienceTables.includes(id)){setPanel(id);return;}
  const region=regions.find(item=>item.id===id);
  if(region)setPanel(region.section);
 },[]);
 ...
 bridge.current.projects=content.data.projects;bridge.current.projectCount=content.data.projects.length;
 ...
 const sectionLabel=section=>section==='personal'?t.journal:t[section];
 const title=panel==='map'?t.map:panel==='settings'?t.settings:panel==='personal'?t.journal:panel==='project-detail'?t.projects:panel==='other-projects'?t.otherProjects:t[panel];
 return <main className={`journey-root ${world.region>4?'journey-night':''} ${touch?'has-touch':''}`} onPointerDownCapture={()=>setBlurred(false)} onClickCapture={releasePointerFocus} onFocusCapture={()=>setBlurred(false)} data-room={world.room||'outdoors'}>
  ...
  {world.loading&&!panel&&<div className="world-status" role="status">{copy.en.loading}…</div>}
  {!ready&&<div className="journey-loading" role="status"><div className="loading-emblem"><Icon name="moon" size={48}/></div><p>{copy.en.brand}</p><h1>{errors.length?copy.en.loadError:copy.en.loading}</h1><div className="loading-track"><span style={{width:`${progress*100}%`}}/></div>{errors.length?<button className="journey-primary" onClick={()=>setVersion(n=>n+1)}>{copy.en.retry}</button>:<small>{Math.round(progress*100)}%</small>}<a href="/">{copy.en.classic}</a></div>}
  ...
  {panel&&!portrait&&<Modal title={title} onClose={close} t={t} wide={panel==='map'} projectPanel={panel==='projects'||panel==='project-detail'||panel==='other-projects'} skillPanel={panel==='skills'}>
   {panel==='map'?<div className="travel-map"><div className="map-directory">{['about','skills',...experienceTables,'projects','personal','awards','contact'].map(section=><button key={section} onClick={()=>setPanel(section)}>{sectionLabel(section)}</button>)}</div>{regions.map(r=><button className={`map-destination ${r.index===world.region?'current':''}`} key={r.id} onClick={()=>travel(r.index)}><div className="map-thumb" style={{backgroundImage:`url(${r.image})`}}><span>{String(r.index+1).padStart(2,'0')}</span><Icon name={r.icon} size={28}/></div><div><small>{sectionLabel(r.section)}</small><h3>{sceneLabel(r,'title')}</h3><p>{sceneLabel(r,'description')}</p></div><Icon name="arrow"/></button>)}</div>:panel==='settings'?<><div className="setting-row"><label htmlFor="touch"><strong>{t.touch}</strong><span>{t.touchSetting}</span></label><input id="touch" type="checkbox" checked={touch} onChange={e=>setTouch(e.target.checked)}/></div><h3>{t.controls}</h3><dl className="controls-list"><div><dt>A / D · ← / →</dt><dd>{t.step}</dd></div><div><dt>SPACE</dt><dd>{t.jump}</dd></div><div><dt>F</dt><dd>{t.fly} / {t.land}</dd></div><div><dt>W / S · ↑ / ↓</dt><dd>{t.ascend}</dd></div><div><dt>Q / E</dt><dd>{t.faceFront} / {t.faceBack}</dd></div><div><dt>ENTER</dt><dd>{t.interact}</dd></div></dl><p>{t.pause}</p><p className="fine-print">{t.saved}<br/>{t.credits}</p><a className="journal-text-button" href="/">{t.classic}<Icon name="external"/></a></>:<ContentPanel section={panel} content={content} lang={lang} t={t} onPersonal={()=>setPanel('personal')} onRestart={()=>travel(0)} projectId={projectId}/>}
  </Modal>}
 </main>;
}
