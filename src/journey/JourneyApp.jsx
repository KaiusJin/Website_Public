...
export default function JourneyApp(){
 ...
 return <main className={`journey-root ${world.region>4?'journey-night':''} ${touch?'has-touch':''}`} onPointerDownCapture={()=>setBlurred(false)} onFocusCapture={()=>setBlurred(false)} data-paused={String(bridge.current.pause)} data-player-x={world.x} data-player-y={world.y} data-player-mode={world.mode} data-velocity-x={world.vx} data-velocity-y={world.vy} data-fps={world.fps} data-room={world.room||'outdoors'} data-region={region.id}>
  ...
  {ready&&started&&!panel&&!portrait&&!world.transitioning&&world.hotspots.map(h=>{const r=regions.find(r=>r.id===h.id)||regions[3];const label=h.kind==='exit'?t.leaveRoom:h.kind==='door'?t.libraryDoor:t[r.section];return <button className={`world-hotspot ${world.near===h.id?'is-near':''}`} key={h.id} ref={node=>{if(node)hotspotNodes.current.set(h.id,node);else hotspotNodes.current.delete(h.id);}} data-world-x={h.worldX} style={{left:h.x,top:h.y}} onClick={()=>openRegion(h.id)} disabled={h.kind==='door'&&(world.near!==h.id||world.mode!=='walking')} aria-label={label}><span className="hotspot-symbol"><Icon name={h.kind==='exit'?'arrow':r.icon} size={23}/></span><span>{label}</span><small>{h.kind==='door'&&world.mode!=='walking'?t.landFirst:world.near===h.id?'E / ↵':'✧'}</small></button>;})}
  {ready&&started&&!panel&&!world.room&&!world.transitioning&&world.atRight&&world.next!==null&&<div className="route-sign route-sign-right">{regionText(regions[world.next],'title',lang)} <Icon name="arrow"/></div>}
  {ready&&started&&!panel&&!world.room&&!world.transitioning&&world.atLeft&&world.previous!==null&&world.previous!==undefined&&<div className="route-sign route-sign-left"><Icon name="arrow"/> {regionText(regions[world.previous],'title',lang)}</div>}
