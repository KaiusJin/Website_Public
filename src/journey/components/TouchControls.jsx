...
function TouchControls({bridge,t,mode}){
 const pad=useRef(null),knob=useRef(null),pointer=useRef(null),bounds=useRef(null);
 const reset=()=>{pointer.current=null;bridge.current.stick={x:0,y:0};if(knob.current)knob.current.style.transform='translate(0px,0px)';};
 useEffect(()=>{window.addEventListener('blur',reset);return()=>{window.removeEventListener('blur',reset);bridge.current.stick={x:0,y:0};};},[bridge]);
 const update=e=>{
